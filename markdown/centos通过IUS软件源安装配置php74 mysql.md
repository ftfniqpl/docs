...
注 2：IUS 跟随上游软件支持周期，当某个版本结束支持后，IUS 会在一个月后将其移至存档库。
如果要安装已结束支持的软件版本，请使用 yum --enablerepo="ius-archive" install php70u-fpm 安装命令（自行替换其中软件包名，下同）。
如果要在存档库里搜索软件，则用 yum --enablerepo="ius-archive" search all php70u 命令。
...
查看软件详情用 yum info 关键词 查看。如果仅限 IUS 源里搜索，则使用 yum --disablerepo="*" --enablerepo="ius" search all 关键词 命令。
```
...
```
yum -y install php74-fpm php74-mysqlnd php74-common php74-cli php74-gd php74-opcache php74-mbstring php74-json php74-pecl-redis php74-process php74-xml php74-pecl-zip php74-bcmath php74-soap php74-xmlrpc php74-devel
```
...
tar -zxvf /var/www/html/latest.tar.gz --strip-components 1 -C /var/www/html
